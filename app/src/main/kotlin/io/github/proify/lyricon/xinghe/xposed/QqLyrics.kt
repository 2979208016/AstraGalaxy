...
/**
 * QQ 音乐在线歌词（参考 LyricProvider 项目 share/cloudlyric + share/qrckit 移植）。
 *
 *  - search：POST musicu.fcg 搜索歌曲，返回候选 (songId, name, artist, duration)
 *  - downloadLyric：POST lyric_download.fcg 取歌词 XML；正文可能是 QRC 加密（16 进制）或普通 LRC
 *  - 解析：QRC 解密（3DES + zlib）后解析 [start,dur] 行；普通 LRC 走 LRC 解析
 */
internal object QqLyrics {
    ...
    data class QqSong(val id: Long, val name: String, val artist: String, val duration: Long)

    /** 搜索歌曲，返回候选列表（已按搜索顺序） */
    fun search(keyword: String, limit: Int = 5): List<QqSong> {
        ...
        val text = httpPost(SEARCH_URL, body) ?: return emptyList()
        val full = runCatching { JSONObject(text) }.getOrNull() ?: return emptyList()
        val songArr = full.optJSONObject("req")?.optJSONObject("data")
            ?.optJSONObject("body")?.optJSONObject("song")?.optJSONArray("list") ?: return emptyList()

        val out = ArrayList<QqSong>(songArr.length())
        for (i in 0 until songArr.length()) {
            ...
            val singerArr = o.optJSONArray("singer")
            val artist = buildString {
                for (j in 0 until (singerArr?.length() ?: 0)) {
                    val s = singerArr?.optJSONObject(j)?.optString("name") ?: continue
                    if (s.isBlank()) continue
                    if (isNotEmpty()) append(" / ")
                    append(s)
                }
            }
            val albumName = o.optJSONObject("album")?.optString("name").orEmpty()
            out.add(QqSong(id, name, artist.ifBlank { albumName }, o.optLong("duration", 0L)))
        }
        return out
    }

    /** 下载并解析歌词；返回 null 表示无歌词 */
    fun downloadLyric(musicId: Long): List<RichLyricLine>? {
        val params = mapOf(
            "version" to "15",
            "miniversion" to "100",
            "lrctype" to "4",
        ...
        val raw = httpPost(LYRIC_URL, postData, contentType = "application/x-www-form-urlencoded") ?: return null

        val content = regexCData(raw, "content") ?: return null
        val decrypted = QrcDecrypt.decrypt(content) ?: return null
        return parseQqContent(decrypted)
    }

    /** 解析 QQ 歌词正文：优先 QRC XML（LyricContent），否则按 LRC 处理 */
    private fun parseQqContent(text: String): List<RichLyricLine>? {
        val trimmed = text.trim()
        if (trimmed.isEmpty()) return null

        // QRC XML 形式：<Qrc LyricContent="..."/>
        val lyricContent = regexLyricContent(trimmed)
        if (!lyricContent.isNullOrEmpty()) {
            val lines = parseQrc(lyricContent)
            if (lines.isNotEmpty()) return lines
        }
        // 普通 LRC
        val lrc = parseLrc(trimmed)
        return if (lrc.isEmpty()) null else lrc
    }

    /** 解析 QRC 行：[start,dur] 文本，行间以换行分隔 */
    private fun parseQrc(content: String): List<RichLyricLine> {
        val out = ArrayList<Pair<Long, String>>()
        val lineRegex = Regex("""\[(\d+)\s*,\s*(\d+)]\s*([^\n\r]*)""")
        for (line in content.lines()) {
            ...
            // 去掉字级时间标签 (text(start,dur))，只保留纯文本
            text = text.replace(Regex("""\s*\((\d+)\s*,\s*\d+\)"""), "")
                .replace(Regex("""<(\d+)\s*,\s*\d+>"""), "")
                .trim()
            if (text.isEmpty()) continue
            if (PLACEHOLDER.any { text.contains(it) }) continue
            out.add(start to text)
        }
        if (out.isEmpty()) return emptyList()
        out.sortBy { it.first }
        return out.mapIndexed { index, (begin, text) ->
            val end = out.getOrNull(index + 1)?.first ?: (begin + 3000L)
    ...
    private fun parseLrc(lrc: String): List<RichLyricLine> {
        ...
        for (line in lrc.lines()) {
            ...
            if (PLACEHOLDER.any { content.contains(it) }) continue
    ...
    private fun regexCData(xml: String, tag: String): String? {
        return runCatching {
            val pattern = Regex("""<$tag[^>]*>.*?<!\[CDATA\[(.*?)]]>""", RegexOption.DOT_MATCHES_ALL)
            pattern.find(xml)?.groupValues?.get(1)?.trim()
        }.getOrNull()
    }

    private fun regexLyricContent(xml: String): String? {
        return runCatching {
            val pattern = Regex("""LyricContent\s*=\s*"([\s\S]*?)"(?=\s*/?>)""")
            val raw = pattern.find(xml)?.groupValues?.get(1) ?: return null
            raw.replace("&lt;", "<").replace("&gt;", ">")
                .replace("&amp;", "&").replace("&quot;", "\"").replace("&#39;", "'")
        }.getOrNull()
    }

    private fun httpPost(url: String, body: String, contentType: String = "application/json; charset=utf-8"): String? {
        return try {
